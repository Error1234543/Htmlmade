...
async function generateQuestions({ topic, numQ, difficulty, language, exam, subject, onProgress, onError }) {
  ...
  const BATCH = isGuj ? 5 : isHin ? 5 : 10;
  const MAX_TOK = isGuj ? 6000 : isHin ? 6000 : 4000;
  // Always use faster/smaller model to reduce token usage
  const MODEL = (total <= 10) ? 'llama-3.1-8b-instant' : 'llama-3.3-70b-versatile';
